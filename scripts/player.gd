...
@export_range(0, 1000, 0.1) var ACCELERATION: float = 800.0
## The overall cap on the character's speed
@export_range(0, 1000, 0.1) var MAX_SPEED: float = 130.0
...
@export_range(0, 1000, 0.1) var FRICTION: float = 700.0
...
## The speed of gravity applied during the downwards motion of a jump
@export_range(0, 10, 0.1) var JUMP_GRAVITY_MULTIPLIER: float = 1.3
## The speed of the jump when leaving the ground
@export_range(0, 1000, 0.1) var JUMP_FORCE: float = 220.0
## The speed of the horizontal movement when leaving the wall
@export_range(0, 1000, 0.1) var WALL_JUMP_FORCE: float = 200.0
## How fast the character's vertical speed goes back to zero when cancelling a jump
@export_range(0, 1000, 0.1) var JUMP_CANCEL_FORCE: float = 500.0
...
func handle_gravity(delta: float) -> void:
	if jumping and velocity.y >= 0:
		velocity.y += JUMP_GRAVITY_MULTIPLIER * GRAVITY * delta
	else:
...
func apply_jump(move_direction: Vector2, jump_force: float = JUMP_FORCE, wall_jump_force: float = WALL_JUMP_FORCE, jump_direction: int = JUMP_DIRECTIONS.UP) -> void:
	can_jump = false
	should_jump = false
	jumping = true
	...
	if (wall_jump):
		...
		velocity.x += wall_jump_force * -move_direction.x
		wall_jump = false
		velocity.y = 0
